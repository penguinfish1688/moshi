...
def _compute_attention_mapped_steering_vector(
    ...
    idx_s = int(torch.argmax(torch.abs(cos_sim_Vs)).item())
...
def _compute_attention_mapped_steering_vector_simple(
    ...
    idx_s = int(torch.argmax(torch.abs(cos_sim_Vs)).item())
    idx_l = int(torch.argmax(torch.abs(cos_sim_Vl)).item())
...
def _extract_all_layer_hidden_from_payload(payload: dict[str, Any]) -> torch.Tensor:
    """Extract all-layer hidden states as [T, L, D].

    Supported payload schemas:
    - text_hidden_layers: [T, L, D]
    - hidden_states: [T, L, D] or [T, D]
    """
    if "text_hidden_layers" in payload:
        hidden = payload["text_hidden_layers"]
        if not isinstance(hidden, torch.Tensor):
        ...
        if hidden.ndim != 3:
            raise ValueError(
                f"Expected text_hidden_layers [T,L,D], got shape {tuple(hidden.shape)}"
            )
        return hidden.float()
    ...
    if "hidden_states" in payload:
        ...
        if hidden.ndim != 3:
            raise ValueError(
                f"Expected hidden_states [T,L,D] or [T,D], got shape {tuple(hidden.shape)}"
            )
        return hidden.float()
...
def _compute_mean_hidden_diff_vectors_from_mode_class_dataset(classifier_dir: str) -> dict[int, torch.Tensor]:
    """Compute per-layer mean diff vectors from mode-class dataset.

    Mode classification windows follow RAON style:
    - listening: question_start + 1 < t < question_end - 1
    - speaking:  question_end + 1 < t < question_end + 11

    where t is token time in seconds (token_idx / frame_rate).
    """
    base = Path(classifier_dir)
    if not base.is_dir():
    ...
    for sd in valid_dirs:
        ...
        if not isinstance(payload, dict):
            raise TypeError(
                f"Expected dict payload in {sd / 'output_hidden.pt'}, got {type(payload).__name__}"
            )

        hidden = _extract_all_layer_hidden_from_payload(payload)  # [T,L,D]
        ...
        if not isinstance(timing_payload, dict):
            raise ValueError(
                f"Expected dict in {sd / 'input_timing.json'}, got {type(timing_payload).__name__}"
            )
        if "question_start" not in timing_payload or "question_end" not in timing_payload:
            raise KeyError(
                f"Missing question_start/question_end in {sd / 'input_timing.json'}"
            )
        ...
        if question_end <= question_start:
            raise ValueError(
                f"Expected question_end > question_start in {sd / 'input_timing.json'}"
            )

        num_steps = int(hidden.shape[0])
        times = torch.arange(num_steps, dtype=torch.float32) / float(frame_rate)
        ...
        print(
            f"[user_interrupt][mode_class] sample={sd.name} "
            f"listening_tokens={sample_listen_count} speaking_tokens={sample_speak_count} total_steps={num_steps}"
        )

        if sample_listen_count > 0:
            chunk = hidden[listen_mask].sum(dim=0).cpu()  # [L,D]
            if listen_sum is None:
                listen_sum = torch.zeros_like(chunk)
            if listen_sum.shape != chunk.shape:
                raise ValueError(
                    f"Listening hidden shape mismatch in {sd}: got {tuple(chunk.shape)}, expected {tuple(listen_sum.shape)}"
                )
            listen_sum += chunk
            listen_count += sample_listen_count

        if sample_speak_count > 0:
            chunk = hidden[speak_mask].sum(dim=0).cpu()  # [L,D]
            if speak_sum is None:
                speak_sum = torch.zeros_like(chunk)
            if speak_sum.shape != chunk.shape:
                raise ValueError(
                    f"Speaking hidden shape mismatch in {sd}: got {tuple(chunk.shape)}, expected {tuple(speak_sum.shape)}"
                )
            speak_sum += chunk
            speak_count += sample_speak_count

    if listen_sum is None or speak_sum is None:
        raise RuntimeError("Failed to collect listening/speaking hidden sums from mode-class dataset")
    if listen_count <= 0 or speak_count <= 0:
        raise RuntimeError(
            f"Insufficient labeled tokens in mode-class dataset: listening={listen_count}, speaking={speak_count}"
        )

    mu_listen = listen_sum / float(listen_count)
    mu_speak = speak_sum / float(speak_count)
    diff = mu_speak - mu_listen  # [L,D]

    print(
        f"[user_interrupt][mode_class] aggregate listening_tokens={listen_count} speaking_tokens={speak_count}"
    )

    vectors: dict[int, torch.Tensor] = {}
    num_layers = int(diff.shape[0])
    ...
    if not vectors:
        raise RuntimeError(
            f"No valid layers found from computed mean diff. Computed layers={num_layers}, "
            f"expected overlap with [{MAIN_LAYER_MIN}..{MAIN_LAYER_MAX}]"
        )
    return vectors
...
def calculate_steering_vector_mean_diff(
    ...
    if mean_hidden_diff_path.exists():
        ...
        print(f"[user_interrupt] Loaded mean-hidden-diff vectors from {mean_hidden_diff_path}")
    else:
        ...
        print(
            f"[user_interrupt] Computed mean-hidden-diff vectors from mode-class dataset and wrote {mean_hidden_diff_path}"
        )
